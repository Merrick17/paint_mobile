...
const DevisScreen = props => {
  ...
  const calculateEstim = () => {
    if (selectedProduct != null) {
        let product = state.productsReducer.find((elm)=>elm._id==selectedProduct); 
        console.log("Prrroood",product)
      if (selectedProduct.symbol == '/') {
          console.log("selected product",selectedProduct)
        const total = (height * width) / Number(product.valeur);
        if (total != NaN) {
            console.log("1")
          return total;
        } else {
